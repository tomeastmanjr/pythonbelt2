...
# def show(request, trip_id):
    ...
    return render(request, 'pythonbelt2app/show.html', context)

def add(request):
    if 'id' not in request.session:
        return redirect(reverse('loginreg:index'))

    return render(request, 'pythonbelt2app/add.html')
...
def create(request):
    ...
    #         return redirect(reverse('pythonbelt2app:show', kwargs={"trip_id":trip.id}))
    # else:
...
#     selected_trip.travelers.add(userid)
    ...
    # return redirect(reverse('pythonbelt2app:show', kwargs={"trip_id":selected_trip.id}))
